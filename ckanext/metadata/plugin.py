...
class MetadataPlugin(p.SingletonPlugin, p.toolkit.DefaultDatasetForm):
    ...
    def _modify_package_schema(self, schema):
        ...
        schema['extras'].update({
            'guid': [ p.toolkit.get_validator('ignore_missing'), unicode ],            
    ...
    def create_package_schema(self):
        log.debug(" plugin - create package schema ")

        schema = super(MetadataPlugin, self).create_package_schema()
          
        log.debug(" plugin - create package schema default ")
        if schema.get('extras'):
             for extra in schema.get('extras'):
                log.debug(" before modify schema %s" % extra )

        schema = self._modify_package_schema(schema)

        log.debug(" plugin - create package schema after _modify ")
        if schema.get('extras'):
             for extra in schema.get('extras'):
                log.debug(" modify schema after  %s" % extra )
