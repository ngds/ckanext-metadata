...
def http_get_content_models():
    ...
    # Use a local copy for now - TODO - autamate a nitely download
...
def package_create(context, data_dict):
    ...
    # log.debug(" metadata - package create 356 context :%r " % data_dict )
  
    new_package = process_extras(context,new_package)
    log.debug(" metadata - package create context  after proc_extra %s " % new_package )
	
    rtnpack = logic.action.create.package_create(context, new_package)

    log.debug(" metadata - package create context - after core pack create " )

    return rtnpack    

def package_update(context, data_dict):
    
    log.debug(" metadata - package update step 1 %r " % data_dict )

    new_package = process_tags(data_dict)
    log.debug(" metadata - package update after process tags " )

    new_package = process_extras(context,new_package)

    log.debug(" metadata - package update after process extras " )

    return logic.action.update.package_update(context, new_package) 

def process_extras(context,new_package):

    log.debug(" metadata - process extras start ")
    if 'md_package' in new_package:
        md_p = json.loads(new_package['md_package'])
        log.debug(" metadata - in proc_extra valid md_package %r" % md_p  )

        m = new_package['publication_date']
        md_date =  time.strftime("%Y-%m-%d %H:%M:%S")

        if new_package['resource_id']:
	    guid = new_package['resource_id'].decode('UTF-8')
        else:

            log.debug(" metadata - in proc_extra  build a new guid %r" % md_p  )

    	    guidm = hashlib.md5()
            log.debug(" metadata - in proc_extra build hers is the hash %s" % guidm  )
            
            gb = ''.join(random.choice(string.ascii_uppercase) for _ in range(16))
               
    	    uniqstr = new_package['title'] + gb 
            log.debug(" metadata - in proc_extra build guid uniqstr %s" % uniqstr  )

    	    guidm.update(uniqstr)

            log.debug(" metadata - in proc_extra build hash update %s" % guidm  )

    	    guid = guidm.hexdigest()

    	extras = []
    	extras.append({'key' : 'guid','value' : guid.decode('UTF-8')})
    	extras.append({'key' : 'spatial-reference-system','value' : ''.decode('UTF-8')}) 
    	extras.append({'key' : 'resource-type','value' : 'dataset'.decode('UTF-8')})
        log.debug(" metadata - process extras  restype" )

        extras.append({'key' : 'metadata-date','value' : md_date.decode('UTF-8')})   

        md_license =  new_package['license_id']
        extras.append({'key' : 'license','value' : md_license.decode('UTF-8')})   

        log.debug(" metadata - process extras lic" ) 
        
        acc_con = []
        extras.append({'key' : 'access_constraints','value' : json.dumps(acc_con)}) 
        extras.append({'key' : 'coupled_resource','value' : json.dumps(acc_con)}) 
      
        ds_cat = "{document:text}"
        extras.append({'key' : 'dataset_category','value' : ds_cat.decode('UTF-8') })

        drefdate = [{ "type":"publication", "value" : m.decode('UTF-8') }]
        extras.append({'key' : 'dataset-reference-date','value' : json.dumps(drefdate)})       
        extras.append({'key' : 'frequency-of-update','value' : ''.decode('UTF-8') }) 
        extras.append({'key' : 'lineage','value' : ''.decode('UTF-8') })
        extras.append({'key' : 'metadata-language','value' : ''.decode('UTF-8') })
        extras.append({'key' : 'spatial-data-service-type','value' : ''.decode('UTF-8') })

        extras.append({'key' : 'progress','value' : 'new'.decode('UTF-8') })

        extras.append({'key' : 'publication_date','value' : m.decode('UTF-8')})
        extras.append({'key' : 'status','value' : 'complete'.decode('UTF-8')})

        md_rc = md_p.get('resourceDescription', {})
        md_maintainer = md_rc.get('resourceContact', {})
        log.debug(" metadata - process extras maintainer %r" % json.dumps(md_maintainer) ) 

        if md_maintainer:
            extras.append({'key' : 'maintainers', 'value' : json.dumps(md_maintainer) })
            maint_dict = md_maintainer[0]
            maint_email = maint_dict['relatedAgent']['agentRole']['contactEmail']
            log.debug(" metadata - maint 1 ")
            if maint_email:
                extras.append({'key' : 'contact-email', 'value' : json.dumps(maint_email) })
            else:
                extras.append({'key' : 'contact-email', 'value' : ''.decode('UTF-8') })
            resp_party = maint_dict['relatedAgent']['agentRole']['organizationName']
            if resp_party:
                rp = [{"name":  json.dumps(resp_party), "roles": ["pointOfContact"]}]
            else:
                rp = [{"name": 'No Organization Name Was Given'.decode('UTF-8'), "roles": ["pointOfContact"]}]
            log.debug(" metadata - maint - 2 - i think it was here ")
            extras.append({'key' : 'responsible-party', 'value' : json.dumps(rp) })
         
        md_author = md_rc.get('citedSourceAgents', {})
        log.debug(" metadata - process extras author %r" % json.dumps(md_author) ) 
        if md_author:
            extras.append({'key' : 'authors', 'value' : json.dumps(md_author) })

        log.debug(" metadata - process extras auth" )

        geoSouth = new_package['md-geo-south']
        geoNorth = new_package['md-geo-north']
        geoEast  = new_package['md-geo-east']
        geoWest  = new_package['md-geo-west']

        geoExt = {}

        if geoSouth:        
            geoExt = {"type":"Polygon",
              "coordinates":[[[ float(geoWest), float(geoSouth)],
                            [ float(geoEast), float(geoSouth)],
                            [ float(geoEast), float(geoNorth)],
                            [ float(geoWest), float(geoNorth)],
                            [ float(geoWest), float(geoSouth)]]]}

            extras.append({'key' : 'spatial', 'value' : json.dumps(geoExt) })
            extras.append({'key' : 'bbox-south-lat', 'value' : geoSouth.decode('UTF-8') })
            extras.append({'key' : 'bbox-north-lat', 'value' : geoNorth.decode('UTF-8') })
            extras.append({'key' : 'bbox-east-long', 'value' : geoEast.decode('UTF-8') })
            extras.append({'key' : 'bbox-west-long', 'value' : geoWest.decode('UTF-8') })
            extras.append({'key' : 'spatial_harvester', 'value' : 'true'.decode('UTF-8') })

        log.debug(" metadata - process extras spatial geo Extent %r" % geoExt )

        # Fix the md_package

        mdharvest_info = md_p.get('harvestInformation', {})
        
        if mdharvest_info:
            if guid: 
                md_p['harvestInformation']['originalFileIdentifier'] = guid.decode('UTF-8')
            else:
            	md_p['harvestInformation']['originalFileIdentifier'] = new_package['resource_id']

            md_p['harvestInformation']['originalFormat'] = 'ISO-USGIN'.decode('UTF-8')
            md_p['harvestInformation']['indexDate'] = md_date.decode('UTF-8')
            md_p['harvestInformation']['version'] = '1.2'.decode('UTF-8')

        md_rc = md_p.get('resourceDescription', {})
        md_rc['resourceDescription'] = md_p.get('notes')

        extras.append({'key' : 'md_package', 'value' : json.dumps(md_p) })
        new_package['extras'] = extras
        del new_package['md_package']

        log.debug(" metadata - process extras return %r " % new_package )
...
def process_extent_old(new_package):
    # dont use this ok 
    geoSouth = new_package.get('md-geo-south')
    geoNorth = new_package.get('md-geo-north')
    geoEast  = new_package.get('md-geo-east')
    geoWest  = new_package.get('md-geo-west')
    geoExt = {'type':'Polygon',
              'coordinates':[[geoWest, geoSouth],
                            [geoEast, geoSouth],
                            [geoEast, geoNorth],
                            [geoWest, geoNorth],
                            [geoWest, geoSouth]]}

    if 'extras' in new_package:
        pkg_extra = new_package['extras']
        pkg_extra.append('spatial',geoExt)
    else:

        pkg_extra = new_package['extras']

    # geometry = json.loads(geoExt)

    # save_package_extent(pkg_id,geometry)
    return pkg_id
